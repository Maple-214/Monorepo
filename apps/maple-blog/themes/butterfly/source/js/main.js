document.addEventListener('DOMContentLoaded', function () {
  ...
  const addHighlightTool = () => {
    ...
    const highlightShrinkEle =
      isHighlightShrink !== undefined ? '<i class="fas fa-angle-down expand"></i>' : '';
  ...
  const addJustifiedGallery = async (ele, tabs = false) => {
    const init = async () => {
      for (const item of ele) {
        ...
        const content =
          item.getAttribute('data-type') === 'url' ? await fetchUrl(text) : JSON.parse(text);
  ...
  const scrollFnToDo = () => {
    ...
    if (isToc) {
      ...
      const tocItemClickFn = (e) => {
        ...
        btf.scrollToDest(
          btf.getEleTop(
            document.getElementById(decodeURI(target.getAttribute('href')).replace('#', '')),
          ),
          300,
        );
